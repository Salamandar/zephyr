	...
	for (uint16_t i = 0; i < NB_LOOP_TEST_SPI; i++) {
		...
		if (status == LR11XX_STATUS_OK) {
			if (version.fw != fw_version_expected) {
				PORTING_TEST_MSG_NOK(" Wrong LR11XX firmware version: expected "
						     "0x%04X / get 0x%04X\n",
		...
		if (status == SX126X_STATUS_OK) {
			if (chip_status.chip_mode == SX126X_CHIP_MODE_UNUSED) {
				PORTING_TEST_MSG_NOK(
					" Wrong SX126X chip mode, get SX126X_CHIP_MODE_UNUSED\n");
				counter_nok++;
			}
		} else {
...
static bool porting_test_radio_irq(void)
{
	...
	if (ral_set_dio_irq_params(&(modem_radio.ral),
				   RAL_IRQ_RX_DONE | RAL_IRQ_RX_TIMEOUT | RAL_IRQ_RX_HDR_ERROR |
...
static return_code_test_t test_get_time_in_s(void)
{
	...
	uint32_t duration = irq_time_s - start_time_s;

	if (duration == (rx_timeout_in_ms / 1000)) {
		PORTING_TEST_MSG_OK();
		SMTC_HAL_TRACE_PRINTF(" Time expected %us / get %us (no margin)\n",
				      (rx_timeout_in_ms / 1000), duration);
	} else {
...
static return_code_test_t test_get_time_in_ms(void)
{
	...
	uint32_t symb_time_ms = (uint32_t)(rx_lora_param.symb_nb_timeout *
					   ((1 << 12) / 125.0)); /* 2^(SF) / BW * symb_nb_timeout */
	...
	uint32_t duration =
		irq_time_ms - start_time_ms - smtc_modem_hal_get_radio_tcxo_startup_delay_ms();
	if (abs(duration - symb_time_ms) <= MARGIN_GET_TIME_IN_MS) {
		PORTING_TEST_MSG_OK();
		SMTC_HAL_TRACE_PRINTF(" Time expected %ums / get %ums (margin +/-%ums)\n",
				      (uint32_t)symb_time_ms, duration, MARGIN_GET_TIME_IN_MS);
	} else {
		PORTING_TEST_MSG_NOK(" Time is not coherent with radio irq : expected %ums / get "
				     "%ums (margin +/-%ums)\n",
				     (uint32_t)symb_time_ms, duration, MARGIN_GET_TIME_IN_MS);
		return RC_PORTING_TEST_NOK;
	}
...
static bool porting_test_timer_irq(void)
{
	...
	uint32_t duration = irq_time_ms - start_time_ms - COMPENSATION_IN_MS_STM32L4;

	if ((duration >= timer_ms) && (duration <= timer_ms + MARGIN_TIMER_IRQ_IN_MS)) {
		PORTING_TEST_MSG_OK();
		SMTC_HAL_TRACE_PRINTF(
			" Timer irq configured with %ums / get %ums (margin +%ums)\n", timer_ms,
			duration, MARGIN_TIMER_IRQ_IN_MS);
	} else {
		PORTING_TEST_MSG_NOK(" Timer irq delay is not coherent: expected %ums / get %ums "
				     "(margin +%ums)\n",
				     timer_ms, duration, MARGIN_TIMER_IRQ_IN_MS);
		return false;
	}
	return true;
}
...
static bool porting_test_stop_timer(void)
{
	...
	while ((smtc_modem_hal_get_time_in_ms() - start_time_ms) < (timer_ms / 2)) {
		/* Do nothing */
	}
	...
	while ((smtc_modem_hal_get_time_in_ms() - start_time_ms) < (timer_ms + 500)) {
		/* Do nothing */
	}
...
static bool porting_test_disable_enable_irq(void)
{
	...
	while ((smtc_modem_hal_get_time_in_ms() - start_time_ms) < (timer_ms + 1000)) {
		/* Do nothing */
	}
...
static bool porting_test_random(void)
{
	...
	if ((rdom1 >= range_min) && (rdom1 <= range_max) && (rdom2 >= range_min) &&
	    (rdom2 <= range_max) && (rdom1 != rdom2)) {
		PORTING_TEST_MSG_OK();
		SMTC_HAL_TRACE_PRINTF(" random1 = %u, random2 = %u in range [%u;%u]\n", rdom1,
				      rdom2, range_min, range_max);
	} else {
	...
	if (ret == true) {
		...
		PORTING_TEST_MSG_WARN(" TODO Warning smtc_modem_hal_get_random_nb_in_range error "
				      "margin > 5%%\n");
	}
...
static bool porting_test_config_rx_radio(void)
{
	...
	for (uint16_t i = 0; i < NB_LOOP_TEST_CONFIG_RADIO; i++) {
		...
		uint32_t duration = smtc_modem_hal_get_time_in_ms() - start_time_ms;

		if (duration >= MARGIN_TIME_CONFIG_RADIO_IN_MS) {
			PORTING_TEST_MSG_NOK(
				" Configuration of rx radio is too long: %ums (margin +%ums)\n",
				duration, MARGIN_TIME_CONFIG_RADIO_IN_MS);
			counter_nok++;
		} else {
...
static bool porting_test_config_tx_radio(void)
{
	...
	for (uint16_t i = 0; i < NB_LOOP_TEST_CONFIG_RADIO; i++) {
		...
		if (ral_set_pkt_payload(&(modem_radio.ral), payload, payload_size) !=
		    RAL_STATUS_OK) {
		...
		if (duration >= MARGIN_TIME_CONFIG_RADIO_IN_MS) {
			PORTING_TEST_MSG_NOK(
				" Configuration of tx radio is too long: %ums (margin +%ums)\n",
				duration, MARGIN_TIME_CONFIG_RADIO_IN_MS);
			counter_nok++;
		} else {
...
static bool porting_test_sleep_ms(void)
{
	...
	uint32_t duration = stop_time_ms - start_time_ms;

	if (abs(duration - sleep_ms) <= MARGIN_SLEEP_IN_MS) {
		PORTING_TEST_MSG_OK();
		SMTC_HAL_TRACE_PRINTF(" Sleep time expected %ums / get %ums (margin +/-%ums)\n",
				      sleep_ms, duration, MARGIN_SLEEP_IN_MS);
	} else {
...
static bool porting_test_timer_irq_low_power(void)
{
	...
	uint32_t duration =
		irq_time_ms - start_time_ms -
		COMPENSATION_IN_MS_STM32L4; /* TODO Warning to compensate delay introduced by */
...
static bool porting_test_flash(void)
{
	...
	SMTC_HAL_TRACE_MSG_COLOR(
		" !! TEST TO BE LAUNCH TWICE !! To check writing after MCU reset\n",
		HAL_DBG_TRACE_COLOR_BLUE);
