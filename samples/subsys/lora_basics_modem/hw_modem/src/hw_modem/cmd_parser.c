...
static const uint8_t host_cmd_tab[CMD_MAX][HOST_CMD_TAB_IDX_COUNT] = {
	...
	[CMD_SET_CLASS] = {1, 1, 1},
	[CMD_CLASS_B_SET_PING_SLOT_PERIODICITY] = {
			1,
			1,
			1,
		},
	[CMD_CLASS_B_GET_PING_SLOT_PERIODICITY] = {
			1,
			0,
			0,
		},
...
cmd_parse_status_t parse_cmd(cmd_input_t *cmd_input, cmd_response_t *cmd_output)
{
	...
	SMTC_HAL_TRACE_INFO("%s (0x%02x)\n", host_cmd_str[cmd_input->cmd_code],
			    cmd_input->cmd_code);
	...
	case CMD_STREAM_STATUS: {
		...
		cmd_output->return_code =
			rc_lut[smtc_modem_stream_status(STACK_ID, &pending_length, &free_length)];

		if (cmd_output->return_code == CMD_RC_OK) {
			cmd_output->length = 4;
			cmd_output->buffer[0] = (pending_length >> 8) & 0xff;
			cmd_output->buffer[1] = pending_length & 0xff;
			cmd_output->buffer[2] = (free_length >> 8) & 0xff;
			cmd_output->buffer[3] = free_length & 0xff;
		}
		break;
	}
	case CMD_LFU_INIT: {
	...
	case CMD_LFU_START: {
		...
		if (file_size != upload_current_size) {
			...
			SMTC_HAL_TRACE_ERROR(
				"Data size uploaded does not correspond to what was defined\n");
				   ...
				   cmd_tst_response_t *cmd_tst_output)
{
	...
	cmd_tst_output->return_code =
		CMD_RC_OK; /* by default the return code is ok and length is 0 */
	cmd_tst_output->length = 0;

#if MODEM_HAL_DBG_TRACE == MODEM_HAL_FEATURE_ON
	SMTC_HAL_TRACE_WARNING("\tCMD_TST_%s (0x%02x)\n",
			       host_cmd_test_str[cmd_tst_input->cmd_code], cmd_tst_input->cmd_code);
#endif
	...
	case CMD_TST_TX_SINGLE: {
		...
		/* integrity of the data shall be tested here as we perform a data
		 * change using LUT before calling api function
		 */
		if (cmd_test_parser_check_sf_bw_cr(raw_sf, raw_bw, raw_cr) == PARSE_OK) {
			...
			cmd_tst_output->return_code = rc_lut[smtc_modem_test_tx(
				NULL, len, freq, pw, sf, bw, cr, 8, false)];
		} else {
	...
	case CMD_TST_TX_CONT: {
		...
		if (cmd_test_parser_check_sf_bw_cr(raw_sf, raw_bw, raw_cr) == PARSE_OK) {
			...
			cmd_tst_output->return_code = rc_lut[smtc_modem_test_tx(
				NULL, len, freq, pw, sf, bw, cr, 8, true)];
		} else {
			/* one of the data is not in the accepted range => reject command */
			cmd_tst_output->return_code = CMD_RC_INVALID;
		}
	...
	case CMD_TST_RX_CONT: {
		...
		if (cmd_test_parser_check_sf_bw_cr(raw_sf, raw_bw, raw_cr) == PARSE_OK) {
			...
			cmd_tst_output->return_code =
				rc_lut[smtc_modem_test_rx_continuous(freq, sf, bw, cr)];
		} else {
			/* one of the data is not in the accepted range => reject command */
			cmd_tst_output->return_code = CMD_RC_INVALID;
		}
	...
	case CMD_TST_RSSI: {
		...
		if (raw_bw < SMTC_MODEM_TEST_BW_COUNT) {
			...
			cmd_tst_output->return_code =
				rc_lut[smtc_modem_test_rssi(freq, bw, time_ms)];
		} else {
	...
	case CMD_TST_TX_SINGLE_PREAM: {
		...
		if (cmd_test_parser_check_sf_bw_cr(raw_sf, raw_bw, raw_cr) == PARSE_OK) {
			...
			cmd_tst_output->return_code = rc_lut[smtc_modem_test_tx(
				NULL, len, freq, pw, sf, bw, cr, preamble_length, false)];
		} else {
			/* one of the data is not in the accepted range => reject command */
			cmd_tst_output->return_code = CMD_RC_INVALID;
		}
