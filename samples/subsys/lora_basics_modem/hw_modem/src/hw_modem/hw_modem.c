...
/**
 * @brief function that will be called every time the COMMAND line in asserted or
 * de-asserted by the host
...
/**
 * @brief function that will be called by the soft modem engine each time an
 * async event is available
 * @param *context  unused context
 * @return none
 */
...
void hw_modem_init(void)
{
	...
	hal_gpio_init_in(HW_MODEM_COMMAND_PIN, BSP_GPIO_PULL_MODE_UP,
			 BSP_GPIO_IRQ_MODE_RISING_FALLING, &wakeup_line_irq);
...
void hw_modem_process_cmd(void)
{
	...
	if (modem_received_buff[0] < 0xFF) {
...
#if 0
		SMTC_HAL_TRACE_ARRAY("Cmd output on uart", modem_response_buff,
			response_length + 2);
#endif
		...
		/* set busy pin to indicate to bridge or host that the hw_modem
		 * answer will be soon sent
		 */
...
void wakeup_line_irq_handler(void *context)
{
	int command_state = hal_gpio_get_value(HW_MODEM_COMMAND_PIN);

	if ((command_state == 0) && (is_hw_modem_ready_to_receive == true)) {
	...
	if ((command_state == 1) && (is_hw_modem_ready_to_receive == false)) {
