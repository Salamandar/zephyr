 ...
 * (The period must be lower than MCU watchdog period (here 32s))
 */
...
void main_hw_modem(void)
{
	...
	while (1) {
		...
		if (hw_modem_is_a_cmd_available() == true) {
			/* Command may generate work for the stack, so drop
			 * down to smtc_modem_run_engine().
			 */
		...
		/* Atomically check sleep conditions (no command available and low
		 * power is possible)
		 *
		 ...
		 *
		 * Even as the code as now, any command received while hal_mcu_set_sleep_for_ms is
		 * active will wait for hal_mcu_set_sleep_for_ms to finish before being processed.
		 * Is this intended?? (up to 20s delay between commands?)
		 */

		/* hal_mcu_disable_irq(); */
		if (!hw_modem_is_a_cmd_available() && hw_modem_is_low_power_ok() &&
