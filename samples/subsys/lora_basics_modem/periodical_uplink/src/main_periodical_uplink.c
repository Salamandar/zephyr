 ...
 * not return SMTC_MODEM_RC_OK
 *
 * @remark The macro is implemented to be used with functions returning
 * a @ref smtc_modem_return_code_t
 *
...
/**
 * @brief Watchdog counter reload value during sleep (The period must be
 * lower than MCU watchdog period (here 32s))
 */
...
/* Flag for button status */
static volatile bool user_button_is_press;
/* uplink raising counter */
...
int main(void)
{
	...
	while (true) {
		...
		hal_watchdog_reload();
		if (!user_button_is_press && !smtc_modem_is_irq_flag_pending()) {
...
/* Schedule a Join LoRaWAN network */
#if defined(USE_RELAY_TX)
			/* by default when relay mode is activated, CSMA is also activated
			 * by default to at least protect the WOR transmission
			...
			/* The RelayModeActivation field indicates how the end-device SHOULD
			 * manage the relay mode.
			 */
			relay_config.activation = SMTC_MODEM_RELAY_TX_ACTIVATION_MODE_DYNAMIC;

			/* number_of_miss_wor_ack_to_switch_in_nosync_mode  field indicates that
			 * the relay mode SHALL be restart in no sync mode when it does not receive
			 * a WOR ACK frame after number_of_miss_wor_ack_to_switch_in_nosync_mode
			 * consecutive uplinks.
			 */
			relay_config.number_of_miss_wor_ack_to_switch_in_nosync_mode = 3;

			/* smart_level field indicates that the relay mode SHALL be enabled
			 * if the end-device does not receive a valid downlink after smart_level
			 * consecutive uplinks.
			 */
			relay_config.smart_level = 8;

			/* The BackOff field indicates how the end-device SHALL behave when it
			 * does not receive a WOR ACK frame.
			 * BackOff Description
		...
		case SMTC_MODEM_EVENT_DOWNDATA:
			...
			/* Get downlink data */
			ASSERT_SMTC_MODEM_RC(smtc_modem_get_downlink_data(
				rx_payload, &rx_payload_size, &rx_metadata, &rx_remaining));
		...
		case SMTC_MODEM_EVENT_FIRMWARE_MANAGEMENT:
			...
			if (current_event.event_data.fmp.status ==
