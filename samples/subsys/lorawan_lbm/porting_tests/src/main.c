...
int main(void)
{
	...
	LOG_INF("");
	LOG_INF("");
	LOG_INF("PORTING_TEST example is starting");
	LOG_INF("");
	LOG_INF("");
	...
	LOG_INF_COLOR(" MCU RESET => relaunch tests and check if read after reset = "
		      "write before reset",
...
static return_code_test_t test_get_time_in_s(void)
{
	...
	if (time == (rx_timeout_in_ms / 1000)) {
		PORTING_TEST_MSG_OK();
		LOG_INF(" Time expected %us / get %us (no margin)", (rx_timeout_in_ms / 1000),
			time);
	} else {
...
static return_code_test_t test_get_time_in_ms(void)
{
	...
	if (abs(time - symb_time_ms) <= MARGIN_GET_TIME_IN_MS) {
		PORTING_TEST_MSG_OK();
		LOG_INF(" Time expected %ums / get %ums (margin +/-%ums)", (uint32_t)symb_time_ms,
			time, MARGIN_GET_TIME_IN_MS);
	} else {
...
static bool porting_test_timer_irq(void)
{
	...
	if ((time >= timer_ms) && (time <= timer_ms + MARGIN_TIMER_IRQ_IN_MS)) {
		PORTING_TEST_MSG_OK();
		LOG_INF(" Timer irq configured with %ums / get %ums (margin +%ums)", timer_ms, time,
...
static bool porting_test_random(void)
{
	...
	if ((rdom1 >= range_min) && (rdom1 <= range_max) && (rdom2 >= range_min) &&
		...
		PORTING_TEST_MSG_OK();
		LOG_INF(" random1 = %u, random2 = %u in range [%u;%u]", rdom1, rdom2, range_min,
			range_max);
	} else {
		PORTING_TEST_MSG_WARN(" => random1 = %u, random2 = %u, expected range [%u;%u]",
	...
	for (uint16_t i = 0; i < tab_size; i++) {
		if (abs(probability_draw - tab_counter_random[i]) > margin) {
			PORTING_TEST_MSG_WARN(" => The number %u has been drawned %u times, "
					      "Expected [%u;%u] times",
...
static bool porting_test_config_rx_radio(void)
{
	...
	for (uint16_t i = 0; i < NB_LOOP_TEST_CONFIG_RADIO; i++) {
		...
		if (time >= MARGIN_TIME_CONFIG_RADIO_IN_MS) {
			PORTING_TEST_MSG_NOK(
				" Configuration of rx radio is too long: %ums (margin +%ums)", time,
	...
	if (counter_nok == 0) {
		...
		PORTING_TEST_MSG_WARN(" => Failed test = %u / %u", counter_nok,
...
static bool porting_test_config_tx_radio(void)
{
	...
	for (uint16_t i = 0; i < NB_LOOP_TEST_CONFIG_RADIO; i++) {
		...
		if (time >= MARGIN_TIME_CONFIG_RADIO_IN_MS) {
			PORTING_TEST_MSG_NOK(
				" Configuration of tx radio is too long: %ums (margin +%ums)", time,
				MARGIN_TIME_CONFIG_RADIO_IN_MS);
			counter_nok++;
		}
...
static bool porting_test_sleep_ms(void)
{
	...
	if (abs(time - sleep_ms) <= MARGIN_SLEEP_IN_MS) {
		PORTING_TEST_MSG_OK();
		LOG_INF(" Sleep time expected %ums / get %ums (margin +/-%ums)", sleep_ms, time,
			MARGIN_SLEEP_IN_MS);
	} else {
		PORTING_TEST_MSG_WARN(" => Sleep time is not coherent: expected %ums / get %ums "
				      "(margin +/-%ums)",
...
static bool porting_test_timer_irq_low_power(void)
{
	...
	/* TODO: Warning to compensate delay introduced by smtc_modem_hal_start_timer for STM32L4 */
	uint32_t time = irq_time_ms - start_time_ms - COMPENSATION_IN_MS_STM32L4;
...
static bool porting_test_flash(void)
{
	...
	LOG_INF("---------------------------------------- %s :", __func__);
	LOG_INF_COLOR(" !! TEST TO BE LAUNCH TWICE !! To check writing after MCU reset",
		      HAL_DBG_TRACE_COLOR_BLUE);
