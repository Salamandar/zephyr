...
static bool modem_in_test_mode;
...
static uint16_t upload_current_size;
static upload_status_t upload_status = UPLOAD_NOT_INIT;

#if defined(CONFIG_LORA_BASICS_MODEM_GEOLOCATION) &&                                               \
	(defined(STM32L476xx) || defined(NRF52840_XXAA))
...
#endif /* CONFIG_LORA_BASICS_MODEM_GEOLOCATION && STM32L476xx */
...
static const uint8_t host_cmd_tab[CMD_MAX][HOST_CMD_TAB_IDX_COUNT] = {
	...
	[CMD_DM_SET_USER_DATA] = {1, SMTC_MODEM_DM_USER_DATA_LENGTH,
				  SMTC_MODEM_DM_USER_DATA_LENGTH},
 ...
 * @return cmd_length_valid_t
 */
static cmd_length_valid_t cmd_test_parser_check_cmd_size(host_cmd_test_id_t test_id,
							 uint8_t length);
...
cmd_parse_status_t parse_cmd(cmd_input_t *cmd_input, cmd_response_t *cmd_output)
{
	...
	case CMD_GET_EVENT: {
		...
		cmd_output->return_code =
			rc_lut[smtc_modem_get_event(&current_event, &event_pending_count)];
		...
		case SMTC_MODEM_EVENT_RESET:
			cmd_output->buffer[2] =
				(uint8_t)(current_event.event_data.reset.count >> 8);
	...
	case CMD_GET_DOWNLINK_DATA: {
		cmd_output->return_code = rc_lut[smtc_modem_get_downlink_data(
			&cmd_output->buffer[2], &cmd_output->buffer[1], &last_dl_metadata,
			&cmd_output->buffer[0])];
	...
	case CMD_GET_ALCSYNC_TIME: {
		...
		cmd_output->return_code =
			rc_lut[smtc_modem_get_alcsync_time(STACK_ID, &gps_time_s)];
		if (cmd_output->return_code == CMD_RC_OK) {
	...
	case CMD_GET_JOIN_EUI: {
		cmd_output->return_code =
			rc_lut[smtc_modem_get_joineui(STACK_ID, &cmd_output->buffer[0])];
		if (cmd_output->return_code == CMD_RC_OK) {
			cmd_output->length = 8;
		}
		break;
	...
	case CMD_SET_JOIN_EUI: {
		cmd_output->return_code =
			rc_lut[smtc_modem_set_joineui(STACK_ID, &cmd_input->buffer[0])];
		break;
	}
	case CMD_GET_DEV_EUI: {
		cmd_output->return_code =
			rc_lut[smtc_modem_get_deveui(STACK_ID, &cmd_output->buffer[0])];
		if (cmd_output->return_code == CMD_RC_OK) {
			cmd_output->length = 8;
		}
		break;
	...
	case CMD_SET_DEV_EUI: {
		cmd_output->return_code =
			rc_lut[smtc_modem_set_deveui(STACK_ID, &cmd_input->buffer[0])];
		break;
	}
	case CMD_SET_NWKKEY: {
		cmd_output->return_code =
			rc_lut[smtc_modem_set_nwkkey(STACK_ID, &cmd_input->buffer[0])];
		break;
	}
	case CMD_SET_CLASS: {
		if (cmd_input->buffer[0] > 2) {
			...
			cmd_output->return_code = rc_lut[smtc_modem_set_class(
				STACK_ID, cmd_modem_class_table[cmd_input->buffer[0]])];
		}
		break;
	}
	case CMD_GET_REGION: {
		cmd_output->return_code =
			rc_lut[smtc_modem_get_region(STACK_ID, &cmd_output->buffer[0])];
		if (cmd_output->return_code == CMD_RC_OK) {
			cmd_output->length = 1;
		}
		break;
	...
	case CMD_SET_REGION: {
		cmd_output->return_code =
			rc_lut[smtc_modem_set_region(STACK_ID, cmd_input->buffer[0])];
		break;
	}
	case CMD_SET_ADR_PROFILE: {
		cmd_output->return_code = rc_lut[smtc_modem_adr_set_profile(
			STACK_ID, cmd_input->buffer[0], &cmd_input->buffer[1])];
		break;
	}
	case CMD_GET_ADR_PROFILE: {
		cmd_output->return_code =
			rc_lut[smtc_modem_adr_get_profile(STACK_ID, &cmd_output->buffer[0])];
		if (cmd_output->return_code == CMD_RC_OK) {
			cmd_output->length = 1;
		}
		break;
	...
	case CMD_GET_NEXT_TX_MAX_PAYLOAD: {
		cmd_output->return_code = rc_lut[smtc_modem_get_next_tx_max_payload(
			STACK_ID, &cmd_output->buffer[0])];
		if (cmd_output->return_code == CMD_RC_OK) {
			cmd_output->length = 1;
		}
		break;
	...
	case CMD_REQUEST_UPLINK: {
		...
		if ((cmd_input->buffer[1] != 0x00) && (cmd_input->buffer[1] != 0x01)) {
			...
			cmd_output->return_code = rc_lut[smtc_modem_request_uplink(
				STACK_ID, cmd_input->buffer[0], cmd_input->buffer[1],
				&cmd_input->buffer[2], cmd_input->length - 2)];
		}
		break;
	}
	...
	case CMD_SET_CERTIFICATION_MODE: {
		cmd_output->return_code =
			rc_lut[smtc_modem_set_certification_mode(STACK_ID, cmd_input->buffer[0])];
		break;
	}
	case CMD_GET_CERTIFICATION_MODE: {
		...
		cmd_output->return_code =
			rc_lut[smtc_modem_get_certification_mode(STACK_ID, &certification_enabled)];
		if (cmd_output->return_code == CMD_RC_OK) {
	...
	case CMD_SET_RTC_OFFSET: {
		...
		LOG_DBG(" change rtc offset to test wrapping with value = %x\n", rtc_offset);
	...
	case CMD_SET_RELAY_CONFIG: {
		...
		user_relay_config.number_of_miss_wor_ack_to_switch_in_nosync_mode =
			cmd_input->buffer[13];
	...
	case CMD_GET_DUTY_CYCLE_STATUS: {
		...
		cmd_output->return_code =
			rc_lut[smtc_modem_get_duty_cycle_status(STACK_ID, &next_free_dtc)];
		if (cmd_output->return_code == CMD_RC_OK) {
	...
	case CMD_SET_DUTY_CYCLE_STATE: {
		cmd_output->return_code =
			rc_lut[smtc_modem_debug_set_duty_cycle_state(cmd_input->buffer[0])];
		break;
	}
	case CMD_GET_ENABLED_DATARATE: {
		...
		cmd_output->return_code =
			rc_lut[smtc_modem_get_enabled_datarates(STACK_ID, &enabled_datarate)];
		if (cmd_output->return_code == CMD_RC_OK) {
	...
	case CMD_SET_NETWORK_TYPE: {
		if (cmd_input->buffer[0] > 1) {
			...
			cmd_output->return_code =
				rc_lut[smtc_modem_set_network_type(STACK_ID, cmd_input->buffer[0])];
		}
		break;
	}
	case CMD_SET_NB_TRANS: {
		cmd_output->return_code =
			rc_lut[smtc_modem_set_nb_trans(STACK_ID, cmd_input->buffer[0])];
		break;
	}
	case CMD_GET_NB_TRANS: {
		cmd_output->return_code =
			rc_lut[smtc_modem_get_nb_trans(STACK_ID, &cmd_output->buffer[0])];
		if (cmd_output->return_code == CMD_RC_OK) {
			cmd_output->length = 1;
		}
		break;
	...
	case CMD_MULTICAST_SET_GROUP_CONFIG: {
		...
		cmd_output->return_code = rc_lut[smtc_modem_multicast_set_grp_config(
			STACK_ID, cmd_input->buffer[0], addr, &cmd_input->buffer[5],
			&cmd_input->buffer[21])];
		break;
	}
	case CMD_MULTICAST_GET_GROUP_CONFIG: {
		...
		cmd_output->return_code = rc_lut[smtc_modem_multicast_get_grp_config(
			STACK_ID, cmd_input->buffer[0], &addr)];
		if (cmd_output->return_code == CMD_RC_OK) {
	...
	case CMD_MULTICAST_CLASS_C_START_SESSION: {
		...
		cmd_output->return_code = rc_lut[smtc_modem_multicast_class_c_start_session(
			STACK_ID, cmd_input->buffer[0], freq, cmd_input->buffer[5])];
		break;
	}
	...
	case CMD_MULTICAST_CLASS_C_STOP_SESSION: {
		cmd_output->return_code = rc_lut[smtc_modem_multicast_class_c_stop_session(
			STACK_ID, cmd_input->buffer[0])];
		break;
	}
	case CMD_MULTICAST_CLASS_C_STOP_ALL_SESSIONS: {
		cmd_output->return_code =
			rc_lut[smtc_modem_multicast_class_c_stop_all_sessions(STACK_ID)];
		break;
	}
	...
	case CMD_ALARM_GET_REMAINING_TIME: {
		...
		cmd_output->return_code =
			rc_lut[smtc_modem_alarm_get_remaining_time(&remaining_time)];
		if (cmd_output->return_code == CMD_RC_OK) {
	...
	case CMD_REQUEST_EMPTY_UPLINK: {
		cmd_output->return_code = rc_lut[smtc_modem_request_empty_uplink(
			STACK_ID, cmd_input->buffer[0], cmd_input->buffer[1],
			cmd_input->buffer[2])];
		break;
	}
	case CMD_LBT_SET_PARAMS: {
		...
		cmd_output->return_code = rc_lut[smtc_modem_lbt_set_parameters(
			STACK_ID, listening_duration_ms, threshold_dbm, bw_hz)];
		break;
	}
	case CMD_LBT_GET_PARAMS: {
		...
		cmd_output->return_code = rc_lut[smtc_modem_lbt_get_parameters(
			STACK_ID, &listening_duration_ms, &threshold_dbm, &bw_hz)];
		if (cmd_output->return_code == CMD_RC_OK) {
	...
	case CMD_LBT_SET_STATE: {
		cmd_output->return_code =
			rc_lut[smtc_modem_lbt_set_state(STACK_ID, cmd_input->buffer[0])];
		break;
	}
	...
	case CMD_CLASS_B_SET_PING_SLOT_PERIODICITY: {
		cmd_output->return_code = rc_lut[smtc_modem_class_b_set_ping_slot_periodicity(
			STACK_ID,
			(smtc_modem_class_b_ping_slot_periodicity_t)cmd_input->buffer[0])];
		break;
	}
	case CMD_CLASS_B_GET_PING_SLOT_PERIODICITY: {
		cmd_output->return_code = rc_lut[smtc_modem_class_b_get_ping_slot_periodicity(
			STACK_ID, &cmd_output->buffer[0])];
		if (cmd_output->return_code == CMD_RC_OK) {
			cmd_output->length = 1;
		}
	...
	case CMD_MULTICAST_CLASS_B_GET_SESSION_STATUS: {
		...
		cmd_output->return_code = rc_lut[smtc_modem_multicast_class_b_get_session_status(
			STACK_ID, cmd_input->buffer[0], &is_session_started,
			&is_session_waiting_for_beacon, &freq, &dr, &ping_slot_periodicity)];
		if (cmd_output->return_code == CMD_RC_OK) {
			cmd_output->buffer[0] = is_session_started;
			cmd_output->buffer[1] = (freq >> 24) & 0xff;
			cmd_output->buffer[2] = (freq >> 16) & 0xff;
	...
	case CMD_MULTICAST_CLASS_B_STOP_SESSION: {
		cmd_output->return_code = rc_lut[smtc_modem_multicast_class_b_stop_session(
			STACK_ID, cmd_input->buffer[0])];
		break;
	}
	case CMD_MULTICAST_CLASS_B_STOP_ALL_SESSIONS: {
		cmd_output->return_code =
			rc_lut[smtc_modem_multicast_class_b_stop_all_sessions(STACK_ID)];
		break;
	}
	case CMD_LORAWAN_GET_LOST_CONNECTION_COUNTER: {
		...
		cmd_output->return_code = rc_lut[smtc_modem_lorawan_get_lost_connection_counter(
			STACK_ID, &lost_connection_cnt)];

		if (cmd_output->return_code == CMD_RC_OK) {
			cmd_output->return_code =
				rc_lut[smtc_modem_lorawan_get_lost_connection_counter_since_s(
					STACK_ID, &lost_connection_since_s)];
		}
		if (cmd_output->return_code == CMD_RC_OK) {
	...
	case CMD_SET_ADR_ACK_LIMIT_DELAY: {
		cmd_output->return_code = rc_lut[smtc_modem_set_adr_ack_limit_delay(
			STACK_ID, cmd_input->buffer[0], cmd_input->buffer[1])];
		break;
	}
	case CMD_GET_ADR_ACK_LIMIT_DELAY: {
		cmd_output->return_code = rc_lut[smtc_modem_get_adr_ack_limit_delay(
			STACK_ID, &cmd_output->buffer[0], &cmd_output->buffer[1])];
		if (cmd_output->return_code == CMD_RC_OK) {
			cmd_output->length = 2;
		}
		break;
	...
	case CMD_GET_LORAWAN_TIME: {
		...
		cmd_output->return_code = rc_lut[smtc_modem_get_lorawan_mac_time(
			STACK_ID, &gps_time_s, &gps_fractional_s)];
		if (cmd_output->return_code == CMD_RC_OK) {
			cmd_output->buffer[0] = (gps_time_s >> 24) & 0xFF;
			cmd_output->buffer[1] = (gps_time_s >> 16) & 0xFF;
			cmd_output->buffer[2] = (gps_time_s >> 8) & 0xFF;
	...
	case CMD_SET_JOIN_DR_DISTRIBUTION: {
		cmd_output->return_code = rc_lut[smtc_modem_adr_set_join_distribution(
			STACK_ID, &cmd_input->buffer[0])];
		break;
	}
	case CMD_LORAWAN_MAC_REQUEST: {
		cmd_output->return_code =
			rc_lut[smtc_modem_trig_lorawan_mac_request(STACK_ID, cmd_input->buffer[0])];
		break;
	}
	case CMD_GET_LINK_CHECK_DATA: {
		...
		cmd_output->return_code =
			rc_lut[smtc_modem_get_lorawan_link_check_data(STACK_ID, &margin, &gw_cnt)];
		if (cmd_output->return_code == CMD_RC_OK) {
	...
	case CMD_DEBUG_CONNECT_WITH_ABP: {
		...
		cmd_output->return_code = rc_lut[smtc_modem_debug_connect_with_abp(
			STACK_ID, dev_addr, &cmd_input->buffer[4], &cmd_input->buffer[20])];
		break;
	}
	case CMD_CSMA_SET_STATE: {
#if defined(LR11XX) || defined(SX126X)
		cmd_output->return_code =
			rc_lut[smtc_modem_csma_set_state(STACK_ID, cmd_input->buffer[0])];
#else
	...
	case CMD_CSMA_GET_PARAMETERS: {
		...
		cmd_output->return_code = rc_lut[smtc_modem_csma_get_parameters(
			STACK_ID, &max_ch_change, &bo_enabled, &nb_bo_max)];
		if (cmd_output->return_code == CMD_RC_OK) {
	...
	case CMD_STREAM_INIT: {
		...
		cmd_output->return_code =
			rc_lut[smtc_modem_stream_init(STACK_ID, port, cipher, redundancy_ratio)];
		cmd_output->length = 0;
		break;
	}
	case CMD_STREAM_ADD_DATA: {
		...
		cmd_output->return_code =
			rc_lut[smtc_modem_stream_add_data(STACK_ID, data, data_len)];
		cmd_output->length = 0;
		break;
	}
	case CMD_STREAM_STATUS: {
		...
		cmd_output->return_code =
			rc_lut[smtc_modem_stream_status(STACK_ID, &pending, &free)];
	...
	case CMD_LFU_INIT: {
		...
		cmd_output->return_code = rc_lut[smtc_modem_file_upload_init(
			STACK_ID, cmd_input->buffer[0], cmd_input->buffer[1], file_store, file_size,
			average_delay)];
		if (cmd_output->return_code == CMD_RC_OK) {
	...
	case CMD_LFU_DATA: {
		...
		if (modem_in_test_mode == true) {
			...
			if ((upload_status != UPLOAD_INIT) &&
			    (upload_status != UPLOAD_DATA_ON_GOING)) {
				cmd_output->return_code = CMD_RC_NOT_INIT;
				LOG_ERR("Upload file data, not init\n");
			} else if (&cmd_input->buffer[0] == NULL) {
				cmd_output->return_code = CMD_RC_NOT_INIT;
				LOG_ERR("Upload file data, null\n");
			} else if ((upload_current_size + cmd_input->length) > file_size) {
				cmd_output->return_code = CMD_RC_INVALID;
				LOG_ERR("Upload file data, size invalid\n");
			} else if (upload_status == UPLOAD_STARTED) {
				cmd_output->return_code = CMD_RC_INVALID;
				LOG_ERR("Upload file still on going\n");
			} else {
				memcpy((uint8_t *)file_store + upload_current_size,
				       &cmd_input->buffer[0], cmd_input->length);
	...
	case CMD_LFU_START: {
		...
		if (file_size != upload_current_size) {
			cmd_output->return_code = CMD_RC_BAD_SIZE;
			LOG_ERR("Data size uploaded does not correspond to what was defined\n");
			...
			LOG_ERR("Bad crc after uploading file data\n");
			smtc_modem_file_upload_reset(STACK_ID);
		} else {
	...
	case CMD_DM_ENABLE: {
		cmd_output->return_code =
			rc_lut[smtc_modem_dm_enable(STACK_ID, cmd_input->buffer[0])];
		break;
	}
	case CMD_DM_GET_PORT: {
		cmd_output->return_code =
			rc_lut[smtc_modem_dm_get_fport(STACK_ID, &cmd_output->buffer[0])];
		if (cmd_output->return_code == CMD_RC_OK) {
			cmd_output->length = 1;
		}
		break;
	...
	case CMD_DM_SET_PORT: {
		cmd_output->return_code =
			rc_lut[smtc_modem_dm_set_fport(STACK_ID, cmd_input->buffer[0])];
		break;
	}
	case CMD_DM_GET_INFO_INTERVAL: {
		...
		cmd_output->return_code =
			rc_lut[smtc_modem_dm_get_info_interval(STACK_ID, &format, &interval)];
		if (cmd_output->return_code == CMD_RC_OK) {
	...
	case CMD_DM_GET_PERIODIC_INFO_FIELDS: {
		cmd_output->return_code = rc_lut[smtc_modem_dm_get_periodic_info_fields(
			STACK_ID, &cmd_output->buffer[0], &cmd_output->length)];
		if (cmd_output->return_code != CMD_RC_OK) {
			cmd_output->length = 0;
		}
		break;
	...
	case CMD_DM_SET_PERIODIC_INFO_FIELDS: {
		cmd_output->return_code = rc_lut[smtc_modem_dm_set_periodic_info_fields(
			STACK_ID, &cmd_input->buffer[0], cmd_input->length)];
		break;
	}
	case CMD_DM_REQUEST_IMMEDIATE_INFO_FIELDS: {
		cmd_output->return_code = rc_lut[smtc_modem_dm_request_immediate_info_field(
			STACK_ID, &cmd_input->buffer[0], cmd_input->length)];
		break;
	}
	case CMD_DM_SET_USER_DATA: {
		cmd_output->return_code =
			rc_lut[smtc_modem_dm_set_user_data(STACK_ID, &cmd_input->buffer[0])];
		break;
	}
	case CMD_DM_GET_USER_DATA: {
		cmd_output->return_code =
			rc_lut[smtc_modem_dm_get_user_data(STACK_ID, &cmd_output->buffer[0])];
		if (cmd_output->return_code == CMD_RC_OK) {
	...
	case CMD_GET_SUSPEND_RADIO_COMMUNICATIONS: {
		...
		cmd_output->return_code =
			rc_lut[smtc_modem_get_suspend_radio_communications(STACK_ID, &suspend)];
	...
	case CMD_SUSPEND_RADIO_COMMUNICATIONS: {
		cmd_output->return_code =
			rc_lut[smtc_modem_suspend_radio_communications(cmd_input->buffer[0])];
		break;
	}
	case CMD_DM_HANDLE_ALCSYNC: {
		cmd_output->return_code =
			rc_lut[smtc_modem_dm_handle_alcsync(STACK_ID, cmd_input->buffer[0])];
		break;
	}
	case CMD_SET_APPKEY: {
		cmd_output->return_code =
			rc_lut[smtc_modem_set_appkey(STACK_ID, &cmd_input->buffer[0])];
		break;
	}
	case CMD_GET_BYPASS_JOIN_DUTY_CYCLE_BACKOFF: {
		...
		cmd_output->return_code =
			rc_lut[smtc_modem_get_join_duty_cycle_backoff_bypass(STACK_ID, &enabled)];
		if (cmd_output->return_code == CMD_RC_OK) {
			cmd_output->buffer[0] = enabled;
			cmd_output->length = 1;
		}
	...
	case CMD_SET_BYPASS_JOIN_DUTY_CYCLE_BACKOFF: {
		cmd_output->return_code = rc_lut[smtc_modem_set_join_duty_cycle_backoff_bypass(
			STACK_ID, cmd_input->buffer[0])];
		break;
	}
	case CMD_MODEM_GET_CRASHLOG: {
		...
		memset(&cmd_output->buffer[crash_string_length + 1], 0,
		       CRASH_LOG_SIZE - crash_string_length);
	...
	case CMD_STORE_AND_FORWARD_GET_STATE: {
		...
		cmd_output->return_code =
			rc_lut[smtc_modem_store_and_forward_get_state(STACK_ID, &state)];
		if (cmd_output->return_code == CMD_RC_OK) {
	...
	case CMD_STORE_AND_FORWARD_CLEAR_DATA: {
		cmd_output->return_code =
			rc_lut[smtc_modem_store_and_forward_flash_clear_data(STACK_ID)];
		break;
	}
	case CMD_STORE_AND_FORWARD_GET_FREE_SLOT: {
		...
		cmd_output->return_code =
			rc_lut[smtc_modem_store_and_forward_flash_get_number_of_free_slot(
				STACK_ID, &capacity, &free_slot)];
		if (cmd_output->return_code == CMD_RC_OK) {
			uint8_t idx = 0;
	...
	case CMD_GNSS_GET_EVENT_DATA_SCAN_DONE: {
		...
		gnss_scan_done_rc =
			rc_lut[smtc_modem_gnss_get_event_data_scan_done(STACK_ID, &gnss_scan_data)];
		...
		if (gnss_scan_done_rc == CMD_RC_OK) {
			...
			cmd_output->buffer[scan_done_offset++] =
				(gnss_scan_data.power_consumption_nah >> 24) & 0xff;
			cmd_output->buffer[scan_done_offset++] =
				(gnss_scan_data.power_consumption_nah >> 16) & 0xff;
			cmd_output->buffer[scan_done_offset++] =
				(gnss_scan_data.power_consumption_nah >> 8) & 0xff;
			cmd_output->buffer[scan_done_offset++] =
				(gnss_scan_data.power_consumption_nah & 0xff);

			cmd_output->buffer[scan_done_offset++] = gnss_scan_data.context.mode;

			cmd_output->buffer[scan_done_offset++] =
				(gnss_scan_data.context.almanac_crc >> 24) & 0xff;
			cmd_output->buffer[scan_done_offset++] =
				(gnss_scan_data.context.almanac_crc >> 16) & 0xff;
			cmd_output->buffer[scan_done_offset++] =
				(gnss_scan_data.context.almanac_crc >> 8) & 0xff;
			cmd_output->buffer[scan_done_offset++] =
				(gnss_scan_data.context.almanac_crc & 0xff);

			cmd_output->buffer[scan_done_offset++] = gnss_scan_data.indoor_detected;

			cmd_output->buffer[scan_done_offset++] =
				(gnss_scan_data.navgroup_duration_ms >> 24) & 0xff;
			cmd_output->buffer[scan_done_offset++] =
				(gnss_scan_data.navgroup_duration_ms >> 16) & 0xff;
			cmd_output->buffer[scan_done_offset++] =
				(gnss_scan_data.navgroup_duration_ms >> 8) & 0xff;
			cmd_output->buffer[scan_done_offset++] =
				(gnss_scan_data.navgroup_duration_ms) & 0xff;

			cmd_output->buffer[scan_done_offset++] =
				(gnss_scan_data.timestamp >> 24) & 0xff;
			cmd_output->buffer[scan_done_offset++] =
				(gnss_scan_data.timestamp >> 16) & 0xff;
			cmd_output->buffer[scan_done_offset++] =
				(gnss_scan_data.timestamp >> 8) & 0xff;
	...
	case CMD_GNSS_GET_SCAN_DONE_RAW_DATA_LIST: {
		...
		if (cmd_output->return_code == CMD_RC_OK) {
			...
			for (uint8_t scan_index = 0; scan_index < gnss_scan_data.nb_scans_valid;
			     scan_index++) {
				cmd_output->buffer[raw_data_offset++] =
					(gnss_scan_data.scans[scan_index].timestamp >> 24) & 0xff;
				cmd_output->buffer[raw_data_offset++] =
					(gnss_scan_data.scans[scan_index].timestamp >> 16) & 0xff;
				cmd_output->buffer[raw_data_offset++] =
					(gnss_scan_data.scans[scan_index].timestamp >> 8) & 0xff;
				cmd_output->buffer[raw_data_offset++] =
					(gnss_scan_data.scans[scan_index].timestamp & 0xff);

				cmd_output->buffer[raw_data_offset++] =
					gnss_scan_data.scans[scan_index].nav_size;

				memcpy(&cmd_output->buffer[raw_data_offset],
				       gnss_scan_data.scans[scan_index].nav,
				       gnss_scan_data.scans[scan_index].nav_size);
	...
	case CMD_GNSS_GET_SCAN_DONE_METADATA_LIST: {
		...
		if (cmd_output->return_code == CMD_RC_OK) {
			...
			for (uint8_t scan_index = 0; scan_index < gnss_scan_data.nb_scans_valid;
			     scan_index++) {
				uint32_t lat_x10000 = (uint32_t)(gnss_scan_data.scans[scan_index]
									 .aiding_position.latitude *
								 10000);
				...
				uint32_t long_x10000 =
					(uint32_t)(gnss_scan_data.scans[scan_index]
							   .aiding_position.longitude *
						   10000);
				...
				cmd_output->buffer[metadata_offset++] =
					(gnss_scan_data.scans[scan_index].scan_duration_ms >> 24) &
					0xff;
				cmd_output->buffer[metadata_offset++] =
					(gnss_scan_data.scans[scan_index].scan_duration_ms >> 16) &
					0xff;
				cmd_output->buffer[metadata_offset++] =
					(gnss_scan_data.scans[scan_index].scan_duration_ms >> 8) &
					0xff;
				cmd_output->buffer[metadata_offset++] =
					(gnss_scan_data.scans[scan_index].scan_duration_ms & 0xff);
			}
			/* At the end of the scan loop the size of the buff is known */
		...
		if (cmd_output->return_code == CMD_RC_OK) {
			...
			for (uint8_t scan_index = 0; scan_index < gnss_scan_data.nb_scans_valid;
				...
				cmd_output->buffer[sv_index++] =
					gnss_scan_data.scans[scan_index].nb_svs;

				for (uint8_t nav_index = 0;
				     nav_index < gnss_scan_data.scans[scan_index].nb_svs;
				     nav_index++) {
					cmd_output->buffer[sv_index++] =
						gnss_scan_data.scans[scan_index]
							.info_svs[nav_index]
							.satellite_id;
					cmd_output->buffer[sv_index++] =
						gnss_scan_data.scans[scan_index]
	...
	case CMD_GNSS_GET_EVENT_DATA_TERMINATED: {
		...
		cmd_output->return_code = rc_lut[smtc_modem_gnss_get_event_data_terminated(
			STACK_ID, &gnss_data_terminated)];
	...
	case CMD_GNSS_SET_CONST: {
		cmd_output->return_code =
			rc_lut[smtc_modem_gnss_set_constellations(STACK_ID, cmd_input->buffer[0])];
		break;
	}
	case CMD_GNSS_SET_PORT: {
		cmd_output->return_code =
			rc_lut[smtc_modem_gnss_set_port(STACK_ID, cmd_input->buffer[0])];
		break;
	}
	...
	case CMD_GNSS_SEND_MODE: {
		cmd_output->return_code =
			rc_lut[smtc_modem_gnss_send_mode(STACK_ID, cmd_input->buffer[0])];
		break;
	}
	...
	case CMD_GNSS_ALM_DEMOD_SET_CONSTEL: {
		cmd_output->return_code = rc_lut[smtc_modem_almanac_demodulation_set_constellations(
			STACK_ID, cmd_input->buffer[0])];
		break;
	}
	case CMD_GNSS_ALM_DEMOD_GET_EVENT_DATA_ALM_UPD: {
		smtc_modem_almanac_demodulation_event_data_almanac_update_t event_data_alm_update =
			{0};
		cmd_output->return_code =
			rc_lut[smtc_modem_almanac_demodulation_get_event_data_almanac_update(
				STACK_ID, &event_data_alm_update)];
		if (cmd_output->return_code == CMD_RC_OK) {
			uint8_t index = 0;
			...
			cmd_output->buffer[index++] =
				(event_data_alm_update.stat_nb_update_from_sat_done >> 24) & 0xff;
			cmd_output->buffer[index++] =
				(event_data_alm_update.stat_nb_update_from_sat_done >> 16) & 0xff;
			cmd_output->buffer[index++] =
				(event_data_alm_update.stat_nb_update_from_sat_done >> 8) & 0xff;
			cmd_output->buffer[index++] =
				(event_data_alm_update.stat_nb_update_from_sat_done & 0xff);

			cmd_output->buffer[index++] =
				(event_data_alm_update.stat_nb_update_from_sat_success >> 24) &
				0xff;
			cmd_output->buffer[index++] =
				(event_data_alm_update.stat_nb_update_from_sat_success >> 16) &
				0xff;
			cmd_output->buffer[index++] =
				(event_data_alm_update.stat_nb_update_from_sat_success >> 8) & 0xff;
			cmd_output->buffer[index++] =
				(event_data_alm_update.stat_nb_update_from_sat_success & 0xff);

			cmd_output->buffer[index++] =
				(event_data_alm_update.stat_nb_aborted_by_rp >> 24) & 0xff;
			cmd_output->buffer[index++] =
				(event_data_alm_update.stat_nb_aborted_by_rp >> 16) & 0xff;
			cmd_output->buffer[index++] =
				(event_data_alm_update.stat_nb_aborted_by_rp >> 8) & 0xff;
			cmd_output->buffer[index++] =
				(event_data_alm_update.stat_nb_aborted_by_rp & 0xff);

			cmd_output->buffer[index++] =
				(event_data_alm_update.stat_cumulative_timings_s >> 24) & 0xff;
			cmd_output->buffer[index++] =
				(event_data_alm_update.stat_cumulative_timings_s >> 16) & 0xff;
			cmd_output->buffer[index++] =
				(event_data_alm_update.stat_cumulative_timings_s >> 8) & 0xff;
			cmd_output->buffer[index++] =
				(event_data_alm_update.stat_cumulative_timings_s & 0xff);

			cmd_output->buffer[index++] =
				(event_data_alm_update.power_consumption_nah >> 24) & 0xff;
			cmd_output->buffer[index++] =
				(event_data_alm_update.power_consumption_nah >> 16) & 0xff;
			cmd_output->buffer[index++] =
				(event_data_alm_update.power_consumption_nah >> 8) & 0xff;
			cmd_output->buffer[index++] =
				(event_data_alm_update.power_consumption_nah & 0xff);
	...
	case CMD_WIFI_GET_SCAN_DONE_SCAN_DATA: {
		...
		cmd_output->return_code = rc_lut[smtc_modem_wifi_get_event_data_scan_done(
			STACK_ID, &wifi_scan_done_data)];
		if (cmd_output->return_code == CMD_RC_OK) {
			uint8_t index = 0;
			...
			cmd_output->buffer[index++] =
				(wifi_scan_done_data.power_consumption_nah >> 24) & 0xff;
			cmd_output->buffer[index++] =
				(wifi_scan_done_data.power_consumption_nah >> 16) & 0xff;
			cmd_output->buffer[index++] =
				(wifi_scan_done_data.power_consumption_nah >> 8) & 0xff;
			cmd_output->buffer[index++] =
				(wifi_scan_done_data.power_consumption_nah & 0xff);

			cmd_output->buffer[index++] =
				(wifi_scan_done_data.scan_duration_ms >> 24) & 0xff;
			cmd_output->buffer[index++] =
				(wifi_scan_done_data.scan_duration_ms >> 16) & 0xff;
			cmd_output->buffer[index++] =
				(wifi_scan_done_data.scan_duration_ms >> 8) & 0xff;
			cmd_output->buffer[index++] = (wifi_scan_done_data.scan_duration_ms & 0xff);

			for (uint8_t scan_index = 0; scan_index < wifi_scan_done_data.nbr_results;
			     scan_index++) {
				/* copy LR11XX_WIFI_MAC_ADDRESS_LENGTH bytes of mac address */
				memcpy(&cmd_output->buffer[index],
				       wifi_scan_done_data.results[scan_index].mac_address,
				       LR11XX_WIFI_MAC_ADDRESS_LENGTH);
				index += LR11XX_WIFI_MAC_ADDRESS_LENGTH;

				cmd_output->buffer[index++] =
					wifi_scan_done_data.results[scan_index].channel;
				cmd_output->buffer[index++] =
					wifi_scan_done_data.results[scan_index].type;
				cmd_output->buffer[index++] =
					wifi_scan_done_data.results[scan_index].rssi;
			}
			cmd_output->length = index;
		}
		break;
	...
	case CMD_WIFI_GET_EVENT_DATA_TERMINATED: {
		...
		cmd_output->return_code = rc_lut[smtc_modem_wifi_get_event_data_terminated(
			STACK_ID, &wifi_data_terminated)];
	...
	case CMD_WIFI_SET_PORT: {
		cmd_output->return_code =
			rc_lut[smtc_modem_wifi_set_port(STACK_ID, cmd_input->buffer[0])];
		break;
	}
	case CMD_WIFI_SEND_MODE: {
		cmd_output->return_code =
			rc_lut[smtc_modem_wifi_send_mode(STACK_ID, cmd_input->buffer[0])];
		break;
	}
	...
	case CMD_LR11XX_RADIO_READ: {
		...
		if (modem_in_test_mode == true) {
			...
			if (lr11xx_hal_read(transceiver_context, command, command_length, data,
					    data_length) != LR11XX_HAL_STATUS_OK) {
				cmd_output->return_code = CMD_RC_FAIL;
			} else {
				cmd_output->return_code = CMD_RC_OK;
			}
	...
	case CMD_LR11XX_RADIO_WRITE: {
		...
		if (modem_in_test_mode == true) {
			...
			if (lr11xx_hal_write(transceiver_context, command, command_length, data,
					     data_length) != LR11XX_HAL_STATUS_OK) {
				cmd_output->return_code = CMD_RC_FAIL;
			} else {
				cmd_output->return_code = CMD_RC_OK;
...
cmd_parse_status_t cmd_test_parser(cmd_tst_input_t *cmd_tst_input,
				   cmd_tst_response_t *cmd_tst_output)
{
	cmd_parse_status_t ret = PARSE_OK;
	...
	if ((cmd_tst_input->cmd_code >= CMD_TST_MAX) ||
		...
		LOG_ERR("Unknown command test (0x%x)\n", cmd_tst_input->cmd_code);
		cmd_tst_output->return_code = CMD_RC_UNKNOWN;
		cmd_tst_output->length = 0;
		return PARSE_ERROR;
	}

	if (cmd_test_parser_check_cmd_size(cmd_tst_input->cmd_code, cmd_tst_input->length) ==
	    CMD_LENGTH_NOT_VALID) {
		LOG_ERR("Invalid size command test (0x%x)\n", cmd_tst_input->cmd_code);
	...
	case CMD_TST_START: {
		if (strncmp((char *)cmd_tst_input->buffer, "TESTTEST", 8) == 0) {
			...
			LOG_ERR("TST MODE: invalid enablement payload\n");
	...
	case CMD_TST_TX_LORA: {
		...
		if (bw < RAL_LORA_BW_010_KHZ) {
			...
			cmd_tst_output->return_code = rc_lut[smtc_modem_test_tx_lora(
				NULL, len, freq, pw, sf, bw, cr, sync_word, invert_iq, crc_is_on,
	...
	case CMD_TST_TX_FSK: {
		...
		cmd_tst_output->return_code =
			rc_lut[smtc_modem_test_tx_fsk(NULL, len, freq, pw, nb_of_tx, delay_ms)];
	...
	case CMD_TST_TX_LRFHSS: {
		...
		cmd_tst_output->return_code = rc_lut[smtc_modem_test_tx_lrfhss(
			NULL, len, freq, pw, cr, bw, grid, enable_hopping, nb_of_tx, delay_ms)];
	...
	case CMD_TST_RX_LORA: {
		...
		cmd_tst_output->return_code = rc_lut[smtc_modem_test_rx_lora(
			freq, sf, bw, cr, sync_word, invert_iq, crc_is_on, header_type,
			preamble_size, nb_symb_timeout)];
	...
	case CMD_TST_READ_NB_PKTS_RX: {
		...
		cmd_tst_output->return_code =
			rc_lut[smtc_modem_test_get_nb_rx_packets(&nb_read_pkt)];
	...
	case CMD_TST_READ_LAST_RX_PKT: {
		...
		cmd_tst_output->return_code = rc_lut[smtc_modem_test_get_last_rx_packets(
			&rssi, &snr, &cmd_tst_output->buffer[4], &rx_payload_length)];
	...
	case CMD_TST_RADIO_READ: {
		...
		cmd_tst_output->return_code = rc_lut[smtc_modem_test_direct_radio_read(
			command, command_length, data, data_length)];
	...
	case CMD_TST_RADIO_WRITE: {
		...
		cmd_tst_output->return_code = rc_lut[smtc_modem_test_direct_radio_write(
			command, command_length, data, data_length)];
		cmd_tst_output->length = 0;
		break;
	}
...
static cmd_length_valid_t cmd_test_parser_check_cmd_size(host_cmd_test_id_t tst_id, uint8_t length)
{
	...
	if (length < host_cmd_test_tab[tst_id][HOST_CMD_TAB_IDX_MIN_LENGTH]) {
		LOG_ERR("Invalid command test size (too small)\n");
		return CMD_LENGTH_NOT_VALID;
	}
	/* cmd len too long */
	if (length > host_cmd_test_tab[tst_id][HOST_CMD_TAB_IDX_MAX_LENGTH]) {
		LOG_ERR("Invalid command test size (too long)\n");
		return CMD_LENGTH_NOT_VALID;
	}
	return CMD_LENGTH_VALID;
}
