...
int initialize_lora(void)
{
	...
	lora_event_handler_loop_thread = k_thread_create(
		&lora_event_handler_loop_thread_data, lora_event_handler_loop_stack_area,
		K_THREAD_STACK_SIZEOF(lora_event_handler_loop_stack_area), lora_event_handler_loop,
...
static int lorademo_ping_send(bool request_not_reply, int counter)
{
	...
	WAIT_FOR(tx_done, PING_TIMEOUT_MS * 1000, apps_common_lr11xx_irq_process(context, IRQ_MASK);
		 k_sleep(K_MSEC(2)));
...
SHELL_STATIC_SUBCMD_SET_CREATE(lorademo,
			       SHELL_CMD_ARG(ping, NULL, "Ping LoRa friend device",
					     cmd_lorademo_ping, 1, 0),
